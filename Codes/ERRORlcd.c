...
int file_count = 0, complete_file_count = 0;
...
struct complete_files{
	char filename[300];
	char fileID[50];
	struct complete_files* cfptr;
};

struct files *root;
struct complete_files *cfroot;
...
void getFileNames(){
	...
	struct dirent *ent;
	struct files *temp;
	struct complete_files *cftemp;
	char nondupes[300],tsplit[300],nsplit[150],*splithash[10],*splitpath[20];
	int i = 0, j = 0;
	
	//raf = fopen("./out.txt","r");
	// temporary
	raf = fopen("./hashcatalog.txt","r");
	
	
	root = (struct files *)malloc(sizeof(struct files));
	temp = root;
	
	cfroot = (struct complete_files *)malloc(sizeof(struct complete_files));
	cftemp = cfroot;
	...
	// save select names into RAM aka disregard ff filetypes
		...
		// split accdg "."; get filename
			...
			strcpy(cftemp->filename,splithash[0]);
			strcat(cftemp->filename,".");
			strcat(cftemp->filename,splitpath[j-1]);
			strcpy(cftemp->fileID,splithash[1]);
			cftemp->cfptr = (struct complete_files *)malloc(sizeof(struct complete_files));
			cftemp = cftemp->cfptr;
...
// FOR WHOLE FILES 
	printf("complete_file_count = %d\n",complete_file_count);

	struct complete_files *cftemp;
	cftemp = cfroot;

	for(i = 0; i < complete_file_count; i++){
		fin = fopen(cftemp->filename,"rb");
		printf("cftemp->filename: %s\n",cftemp->filename);
		char holder[300];

		strcpy(holder,file_agent_dir);
		strcat(holder,chunk_storage_dir);
		strcat(holder,"/");
		strcat(holder,cftemp->fileID);
		printf("\n=======%s=",holder);
		fout = fopen(holder,"wb");

		int size=getSize(cftemp->filename);
		char buffer[20];
		char string[size];
		fread(string, 1, size, fin);;
		fwrite(string,1,size,fout);
		cftemp = cftemp->cfptr;
		fclose(fout);
		fclose(fin);
	}
