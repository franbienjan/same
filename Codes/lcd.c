...
void getFileNames(char *str){
	...
	// split accdg "."; get filetype
		...
		printf("splitpath: %s\n",splitpath[j-1]);
		printf("j = %d\n",j);
		printf("newnondupes = %s\n",nondupes);
		
	// save select names into RAM aka disregard ff filetypes
		//printf("splitpath: %s\n",splitpath[j-1]);
		if(strcmp(splitpath[j-1],"DS_Store") && strcmp(splitpath[j-1],chunk_storage_dir) && strcmp(splitpath[j-1],"bmp") && strcmp(splitpath[j-1],"png") && strcmp(splitpath[j-1],"jpg") && strcmp(splitpath[j-1],"jpeg") && strcmp(splitpath[j-1],"gif") && strcmp(splitpath[j-1],"zip") && strcmp(splitpath[j-1],"rar") && strcmp(splitpath[j-1],"tar") && strcmp(splitpath[j-1],"tiff") && strcmp(splitpath[j-1],"svg") && strcmp(splitpath[j-1],"mp3") && strcmp(splitpath[j-1],"mp4") && strcmp(splitpath[j-1],"mkv") && strcmp(splitpath[j-1],"exe") && strcmp(splitpath[j-1],"mp3")){
		...
		// split accdg "."; get filename
			...
			strcpy(nsplit,".");
			strcat(nsplit,splitpath[0]);
			strcat(nsplit,".");
			for(i=1;i<j;i++){
				strcat(nsplit,splitpath[i]);
				if(i != j-1){
					strcat(nsplit,".");
				}
			}
			...
			char c[strlen(nondupes)+4+strlen(splitpath[j-1])];
			strcpy(c,".");
			strcat(c,splitpath[0]);
			strcat(c,".");
			for(i=1;i<j;i++){
				strcat(c,splitpath[i]);
				if(i != j-1){
					strcat(c,".");
				}
			}
...
void letMeFree(char** pac){
	int i,gg=0;
	
	printf("\nSTART");
	
	while(pac[gg]!=NULL) gg++;
	
	for(i=0 ; i<gg ; i++) free(pac[i]);
...
///////////// Check if this is cause of error
